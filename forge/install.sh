...
echo "Installing Forge to $TARGET..."

# Create directories
mkdir -p "$TARGET/.claude/skills/forge"
mkdir -p "$TARGET/.claude/commands/Forge"

# Copy skill files (resources, SKILL.md)
cp -R "$SCRIPT_DIR/resources" "$TARGET/.claude/skills/forge/"
cp "$SCRIPT_DIR/SKILL.md" "$TARGET/.claude/skills/forge/"

# Copy command files
cp "$SCRIPT_DIR/commands/"*.md "$TARGET/.claude/commands/Forge/"
...
echo ""
echo "Done! Forge is installed."
echo ""
echo "  Skill:    $TARGET/.claude/skills/forge/"
echo "  Commands: $TARGET/.claude/commands/Forge/"
echo ""
echo "Run /Forge:brainstorm to start designing a new skill."
